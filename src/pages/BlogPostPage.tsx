...
import { getBlogPost, getTherapy, getProfessionalsByTherapy } from "@/lib/api";
...
export default function BlogPostPage() {
  ...
  const relatedPros = getProfessionalsByTherapy(post.therapyId);

  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        <Link to="/blog" className="text-terracotta text-sm font-medium inline-flex items-center gap-1 mb-6 hover:underline">
          <ArrowLeft size={16} /> {t("blog.back")}
        </Link>

        {therapy && (
          <Link to={`/therapies/${therapy.id}`} className="text-xs font-medium text-terracotta bg-terracotta/10 px-3 py-1 rounded-full">
            {therapy.name[locale]}
          </Link>
        )}

        <h1 className="text-3xl font-bold text-brown mt-4 mb-6">{post.title[locale]}</h1>
